/**
 * Copyright (C) Deepa Sysisoft Pvt Ltd (2015-2017). All Rights Reserved. 
 * This code may not be copied or used without permission.
 */
package dao;
...
public class JDBCUserRepository implements UserRepository {
  ...
  public void notifyMeOnOpen() {
	...
	@Override
	public CompletionStage<Optional<User>> findByUsernameAndPassword(String username, String password) {
	...
	public Stream<User> jdbcFind(String loggedInUsername) {
		String sql = "select user.id as id,user.username as username,user.name as name,user.alias as alias,user.enabled as enabled"
				+ " from user user ";
	...
	public Optional<User> jdbcFindUsingSql(String sql, List<String> args) {
		...
		try {
			...
			Optional<List<String>> optionalAuthorities = jdbcReadUserAuthorities(user.getUsername(), user.getUsername());
	...
	public Optional<User> jdbcFindByUsername(String username, String loggedInUsername) {
		String sql = "select user.id as id,user.username as username,user.name as name,user.email as email, user.alias as alias,user.enabled as enabled"
	...
	public Optional<User> jdbcReadUser(long userId, String loggedInUsername) {
		...
		try {
			...
			List<String> authorities=jdbcReadUserAuthorities(user.getUsername(), loggedInUsername).get();
	...
	public Optional<List<String>> jdbcReadUserAuthorities(String username, String loggedInUsername) {
		String sql = "select auth.username as username,auth.authority as authority"
	...
	public Optional<User> jdbcAddUser(User user, String loggedInUsername) {
	  ...
	  public Optional<User> jdbcUpdateUser(User user, String loggedInUsername) {
		...
		String sql =
            "update user set password=?, email=?, name=?, alias=?) where id=?";
	...
	public Optional<User> jdbcDeleteUser(long userId, String loggedInUsername) {
		String sql = "delete from user where id=? and " + " exists (select 1 from authorities where username=? and"
