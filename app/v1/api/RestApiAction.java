...
public class RestApiAction extends play.mvc.Action.Simple {
  private final Logger.ALogger logger = play.Logger.of("application.RestApiAction");
  ...
  @Singleton
  @Inject
  public RestApiAction(MetricRegistry metrics, HttpExecutionContext ec, Futures futures) {
    this.ec = ec;
    this.futures = futures;
    this.requestsMeter = metrics.meter("requestsMeter");
    this.responsesTimer = metrics.timer(name(RestApiAction.class, "responsesTimer"));
  }
  ...
  public CompletionStage<Result> call(Http.Context ctx) {
    ...
    if (ctx.request().accepts("application/json")) {
      ...
      return futures
          .timeout(doCall(ctx), 1L, TimeUnit.SECONDS)
          .exceptionally(
              e -> {
                // return (Results.status(GATEWAY_TIMEOUT, views.html.timeout.render()));
                return status(GATEWAY_TIMEOUT, "Database timed out");
              })
